...
public class CriterionTranslator {
    public static MutableComponent tryTranslateCriterion(Advancement advancement, String criterion) {
        String preppedCriterion = criterion.replace(":",".");
        if (!criterion.contains(":")) preppedCriterion = advancement.getId().getNamespace()+ "." + preppedCriterion;
        List<String> translations = new LinkedList<>();
        translations.add(preppedCriterion);
        translations.add("betteradvancements.criterion." + advancement.getId() + "." + criterion);
